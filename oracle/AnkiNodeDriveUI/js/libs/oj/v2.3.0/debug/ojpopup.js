 ...
 */(function ()
{
   ...
   * </h3>
   *
   * <p>The <code class="prettyprint">:oj-popup</code> pseudo-selector can be used in jQuery expressions
   *    to select JET Popups.  For example:</p>
   *
   * <pre class="prettyprint">
        ...
        else
        {
          ...
          if (props["target"] && props["target"]["height"] === 0 && props["target"]["width"] === 0)
          {
            ...
            if (!oj.StringUtils.isEmptyOrUndefined(myrule))
            {
              ...
              var suggestedHrule = "center" ===  myrules[0] ? myrules[0] : props['horizontal'];
      ...
      'ojPopup' :
        {
          'modality' : oj.Components.createDynamicPropertyGetter(
            function ()
            {
              return (oj.ThemeUtils.parseJSONFromFontFamily('oj-popup-option-defaults') || {})["modality"];
            })
        }
    });
}
...
oj.Components.registerMetadata('ojPopup', 'baseComponent', ojPopupMeta);
oj.Components.register('oj-popup', oj.Components.getMetadata('ojPopup'));
})();
});
