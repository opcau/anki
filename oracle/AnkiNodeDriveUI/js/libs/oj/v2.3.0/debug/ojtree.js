...
(function ()
{
  ...
  */
  ...
  //  ojTree class names
  var   /** @const */   OJT_TREE          = "oj-tree",                 // a tree
        /** @const */   OJT_NODE          = "oj-tree-node",            // a tree node (parent or leaf)
        ...
        /** @const */   OJT_TITLE         = "oj-tree-title",           // node text
        /** @const */   OJT_TYPE          = "oj-tree-type",            // node has a user-defined type
  ...
  var  /** @const */  TRANSKEY_LOADING  = "stateLoading" ;
  var  /** @const */  TRANSKEY_NEWNODE  = "labelNewNode" ;
  var  /** @const */  TRANSKEY_MULTISEL = "labelMultiSelection" ;
  ...
  $(function() {
     if (/msie/.test(navigator.userAgent.toLowerCase())) {
       e1 = $('<textarea cols="10" rows="2"></textarea>').css({ position: 'absolute', top: -1000, left: 0 }).appendTo('body');  //@HTMLUpdateOK
       e2 = $('<textarea cols="10" rows="2" style="overflow: hidden;"></textarea>').css({ position: 'absolute', top: -1000, left: 0 }).appendTo('body'); //@HTMLUpdateOK
       scrollbar_width = e1.width() - e2.width();
       e1.add(e2).remove();
     }
     else {
       e1 = $('<div />').css({ width: 100, height: 100, overflow: 'auto', position: 'absolute', top: -1000, left: 0 })
                        .prependTo('body').append('<div />').find('div').css({ width: '100%', height: 200 });    //@HTMLUpdateOK
...
//        this["after_open"](node);
     ...
     */
     ...
     getContextByNode: function(node)
     {
        ...
        node  = (node == null || node == undefined)? 0 : node ;  // don't give getNode() null/undefined
        node  = this._getNode(node) ;  // validate node reference (returns jQ wrapped node, -1, null)
        bNode = (node != -1 && (node.length > 0)) ;
        bTree = (node == -1) ;
     ...
     scrollIntoView : function (obj, alignTo, setActive)
     {
        ...
        toTop     = (alignTo == 'top') ;     // default is 'bottom'
     ...
     _compareSelectionValues : function(value1, value2)
     {
       ...
       len  = (value1 && value1.length)? value1.length : 0 ;
       len2 = (value2 && value2.length)? value2.length : 0 ;
...
//        this._lock(newval) ;
     ...
     _handleSelectionOptionChange : function(newSels)
     {
        ...
        var newvals ;
        var _this  = this ;
        var $elem, id, inList, len, bRemoved = false;
        ...
        // selections array.
        newvals = $.grep(sels, function(node, i) {
                                  $elem = _this._getNodeElem(node) ;
                                  if ($elem) {                       // if node elem exists
                                    cur.push($elem) ;                // note it.
                                    sels[i] = $elem[0] ;             // ensure we store an <li>
                                  }                                  // not a string like "#news"
                                  else {
     ...
     _prepare_move :  function (o, r, pos, cb, is_cb)
     {
       ...
       if (!is_cb && this._data.core.prepMoveBlk.o                &&
...
//     this._emitEvent(this._data.core.prepMoveBlk, "prepare_move", true);
     ...
     /**
       ...
       *   @private
       */
     _moveNode : function (obj, ref, position, is_copy, is_prepared, skip_check)
     {
        if (! is_prepared) {
          obj = this._getNode(obj) ;
        }
...
*/
      ...
      _cleanNode  : function (obj)
      {
         var dndClasses,
             draggableAttr,
             draggableAttrVal,
             dndContext = this._getDndContext(),
             cons       = oj.TreeDndContext ;
      ...
      /**
        ...
        *  @private
        */
      _createNode :  function (obj, position, js, callback, is_loaded)
      {
         obj = obj || -1 ;
...
//         }
    ...
    /**
      ...
      *  @private
      */
    _expandAll  : function (obj, animate, original_obj)
    {
      ...
      obj.each(function ()
           {
             ...
             if (!_this._is_loaded(this))   {
               _this["expand"](this, function()
                                     {
                                       _this._expandAll(__this, animate, original_obj);
                                     }, !animate);
             }
             else  {
    ...
    _select : function (node, check, e)
    {
       var  core        = this._data.core,
            ui          = this._data.ui,
            selectMode  = core.selectMode;      // 0 = no select, 1 to n, or -1 = multi-select
...
//   },
    ...
    /**
     ...
     _getSelected : function (context)
     {
         ...
         sel = context ? $(context).find("a.oj-selected").parent() : this._data.ui.selected ;
         ar  =[] ;
         len = sel.length ;
         for (i = 0; i < len; i++) {
            ar.push(sel[i]) ;
     ...
     /**
       ...
       *  overriden by data methods such as _is_loaded_J(), _is_loaded_html()
       *  or _is_loaded_DS().
       *  @private
       */
...
//      if (obj && obj !== -1 && this._data.ds.progressiveUnload && ($.isFunction(s["data"]) || !!s["ajax"]))  {
     ...
     {
       var _this = this;
       this._loadNodeHtml(obj, function ()                   // success func
                                {
                                  _this._emitEvent({ "obj" : _this._getNode(obj)}, "load_node", true) ;
                                  s_call.call(this);
                                },
...
*/
    ...
    /**
     ...
     /**
       *  Load json for a particular node (or -1 for the whole tree). s_call and e_call are
       *  optional success and error callback's.
       *  @private
       */
     _loadNodeJson : function (obj, s_call, e_call)
     {
          ...
          case (!data && !!ajax) || (!!data && !!ajax && obj && obj !== -1):
                       ...
                       success_func = function (d, status, x)
                       {
                         ...
                         if (typeof d == "string") {
                           ...
                           if (! tempd) {
                             return error_func.call(this, x, "Bad JSON", "");
                           }
                         }
...
//              }
                ...
                if (! bIns) {
                  ...
                  tmp.prepend("<ins class='oj-tree-icon " + OJT_NICON + " oj-component-icon" + "'>&#160;</ins>", sp);    // node icon //@HTMLUpdateOK
                  bIns = true ;
                }
                if (!m["icon"] && js["icon"])  {
                  m["icon"] = js["icon"];
           ...
           d.append(tmp);            // append the <a> and its children to the <li>                    //@HTMLUpdateOK
      ...
      *   @private
      */
     __getJson : function (obj, li_attr, a_attr, isCallback)
     {
       var result = [],
           s      = this.options,
           _this  = this,
...
//             }
             ...
             a.each(function ()  {
                                                                                ...
                                                                                .replace(/oj-tree[^ ]*|$/ig,'').length))
                 {
                   ...
                   $.each(a_attr, function (k, z) {
                       sAttr = (" " + (t.attr(z) || "")).replace(/ oj-tree[^ ]*/ig,'')
                                                        .replace(/ oj-[^ ]*/ig,'')
                                                        .replace(/\s+$/ig," ")
                                                        .replace(/^ /,"")
                                                        .replace(/ $/,"");
             ...
             li = li.find("> ul > li");
             if (li.length)  {
                tmp1.children = _this.__getJson(li, li_attr, a_attr, true);
             }
             result.push(tmp1);
       });
...
*/
     ...
     /**
       ...
       *   @private
       */
     _loadNodeHtml : function (obj, s_call, e_call)
     {
       var s = this._getOptions()["data"],        // work on a copy of the options to avoid
           d,                                     // endless loop in calling success/error funcs
           error_func   = function () {},
          ...
          case (!data && !ajax):
                    if (!obj || obj == -1 )  {
                      this._$container_ul
                              ...
                              .prepend("<ins class='oj-tree-icon' >&#160;</ins>").end()          //@HTMLUpdateOK
         ...
         if (this._data.core.toLoad.length) {
            ...
            if (current.length) {
               ...
               $.each(current, function (i, val) {
                   if (! _this._is_loaded(val))  {        // if load not loaded,
...
//      this._initCrrm() ;          // not in V1
     ...
     _emitEvent : function (data, evname, bInternal)
     {
        ...
        var func,
            ...
            isBefore = (evname === "before"),
            isPublic = bInternal? false : true ;
        ...
        if (isPublic) {
          ...
          if (isBefore) {
            ...
            if (typeof bContinue != "undefined") {
              bContinue = bContinue? true : false ;         // returns true/false/undefined
            }
            return bContinue ;
          }
        }
                    ...
                    $.each(types, function (i, tp)
                       {
                         ...
                         if ( tp["icon"]["image"] || tp["icon"]["position"])  {
                           if (i == "default")  {
                             ...
                             icons_css += '.oj-tree-' + _this._getIndex() + ' .oj-tree-list li.oj-tree-type a > .oj-tree-node-icon { ';
                             icons_css += _this._addTypeCss(tp, icons_css) ;
                             icons_css += '} ';
                             icons_css += '.oj-tree-' + _this._getIndex() + ' .oj-tree-list li[' + attr + '="oj-tree-deftype"].oj-tree-type > a ins.oj-tree-node-icon { ';
                           }
                           else if (tp["icon"]["image"]) {
                             icons_css += '.oj-tree-' + _this._getIndex() + ' .oj-tree-list li[' + attr + '="' + i + '"].oj-tree-type > a > ins.oj-tree-node-icon { ';
                           }
...
//         this["rename"](this._data.ui.hovered || this._data.ui.lastSelected);
     ...
     _crrm_move_node : function (obj, ref, position, is_copy, is_prepared, skip_check)
     {
        ...
        this._moveNode(obj, ref, position, is_copy, true, skip_check);
     },
...
//          w   = this.options["crrm"]["inputWidthLimit"],       // applyDefaults() not done for V1
            ...
            h2  = obj.css("position", "relative").append(         //@HTMLUpdateOK
                $("<input />", {
                    "value" : t,
...
_this._done = false ;
     ...
     _crrm_create : function (obj, position, js, callback, skip_rename)
     {
        ...
        this.__rollback();
        t = this._createNode(obj, position, js, function (t)
             {
...
//   },
     ...
     _getString: function(key)
     {
       ...
       s = oStrings[key] ;                          // have we already escaped this string?
       if (! s) {                                   // no
     ...
     {
        var ret = 0, val, c, s, $div, span ;

        if (window.getComputedStyle) {
          $div = this._data.ui.animDurDiv ;
          if (! $div) {
          ...
          span = $div[0].childNodes[0] ;
          val = window.getComputedStyle(span);
          ...
          if (typeof val == "string" && val.length > 1) {
            c = val.charAt(val.length -1).toLowerCase()
            if (c == "s") {
              ...
              if (! isNaN(val)) {
               ...
               if (val.length > 2) {
                 s = val.substring(val.length - 2).toLowerCase() ;
                 if (s == "ms") {
...
oj.TreeDndContext.prototype.initDnDOpts = function()
{
  ...
  if (! dnd.reorder)
  {
    if ((! dnd.dragDataTypes) && (! dnd.dragStartCallback))
    {
      ...
      dnd.dragStartCallback =
      dnd.dragDragCallback  =
      dnd.dragEndCallback   =
    ...
    if (! dnd.dropDropCallback)
    {
      ...
      dnd.dropDragEnterCallback = 
      dnd.dropDragOverCallback  = 
      dnd.dropDragLeaveCallback = 
      dnd.dropDropCallback      = 
...
oj.TreeDndContext.prototype._dragEnter =  function(e)
{
   ...
   {
     return ;   // (prevents bogus insert line under top node)
   }
   ...
   {
     this._dndHideMarker() ;
     return ;                  // vetoed - drag over self ignored, or don't have a node
   }
...
oj.TreeDndContext.prototype._dragOver =  function(e)
{
  ...
  $node = vars.r ;           // the reference node set by _dragEnter()
...
//this._dndDrag(e) ;
  ...
  if (! this._dndPrepare())        // returns "after", "before", "inside" if
  {                                // drop position is acceptable
    this._dndHideMarker() ;
    return ;
  }
  ...
  {
    ret = dnd.dropDragOverCallback(e, {"item"      : dnd.bInternalNode? null : $node, 
                                       "reference" : dnd.bInternalNode? null : vars.r,
...
oj.TreeDndContext.prototype._dragDrop =  function(e)
{
   ...
   var vars       = dnd.vars ;
   var dt         = e.originalEvent.dataTransfer ;
   var foreignObj = (typeof vars.o == 'boolean' && vars.o) ;  // true if not a tree node
   var veto       = false ;
   var ret ;
   ...
   if (vars.r)           // sanity check - ensure we have a ref node drop position
   {
     if (dnd.dropDropCallback)
     {
       ret = dnd.dropDropCallback(e, {"reference" : dnd.bInternalNode? null : vars.r,
                                      "position"  : dnd.bInternalNode? "first" : vars.last_pos,
...
/**
  ...
  *  @private
  */
oj.TreeDndContext.prototype._dndEnter = function (obj)
{
   var dnd  = this._dnd ;
   var vars = dnd.vars ;
   var cons = oj.TreeDndContext ;
...
oj.TreeDndContext.prototype._dndPlacement =  function (e, $node)
{
   ...
   if (! $node.hasClass(TreeUtils._OJ_TEMPNODE)) {
     ...
     dnd.place = (e.pageY - (dnd.off.top || 0)) ;
     dnd.place = (dnd.place >= this.component._getNodeHeight())?  "after"  :
                 (dnd.place < 0)?  "before"  : "inside" ;
...
oj.TreeDndContext.prototype._dndPrepare = function ()
{
   ...
   else
   {
     ...
     if (this.component._is_loaded(vars.r))
     {
...
oj.TreeDndContext.prototype._dndShowMarker = function()
{
   ...
   pos       = rtl? (dnd.off.right - 18) : (dnd.off.left + 5);
   ...
   switch (r)
   {
      ...
      case "inside":       // marker only, no line
          this._moveMarkerUI(vars.m, {"left" : mLeft + (rtl? (isParent? -4 : 0) : 4),
