...
function Speedometer(Element) {
  ...
  //if (!Container) throw ('speedometer container not found: '+Element); // XXX
  ...
  var Size = Math.min (
    ...
    */
  var Size = (document.body.clientWidth/4)-30;
  ...
  // Threshold 
  var ThresholdLow   = options.thresholdlow      || 50.0;
  var ThreshPivotLow = options.thresholdPivotLow || 25.0;
  var ThresholdMed   = options.thresholdmed      || 30.0;
  var ThreshPivotMed = options.thresholdPivotMed || 65.0;
  var ThresholdHigh  = options.thresholdhigh      || 20.0;
  var ThreshPivotHigh = options.thresholdPivotHigh || 90.0;
  ...
  var Color = {
    ...
    rim   : theme.rim,
    rimArc: theme.rimArc,
    threshlow: theme.threshlow,
    threshmed: theme.threshmed,
    threshhigh: theme.threshhigh,
    center: theme.center,
    nose  : theme.nose,
    hand  : {
      main   : theme.hand,
  ...
  this.drawBackground = function ()
  {
    ...
    // Draw Threshold low
    context.strokeStyle = Color.threshlow;
    context.lineWidth = Size / 50;

    var val = MaxValue - MinValue
    val = (MaxValue * (ThreshPivotLow - MinValue)) / val;
    val = ((MeterToAngle - MeterFromAngle) * val) / MaxValue;
    val += MeterFromAngle;

    var stAngle = val - ((MeterRimAngle * ThresholdLow) / MaxValue / 2);
    if (stAngle <= MeterFromAngle)
      stAngle = MeterFromAngle;

    var sweepAngle = ((MeterRimAngle * ThresholdLow) / MaxValue);
    ...
    context.strokeBoxedArc (x + gap, y + gap, w - gap * 2, h - gap * 2,
                            ...
                            /* counterclockwise = */ false);
    // Draw Threshold med
    context.strokeStyle = Color.threshmed;
    context.lineWidth = Size / 50;

    var val = MaxValue - MinValue
    val = (MaxValue * (ThreshPivotMed - MinValue)) / val;
    val = ((MeterToAngle - MeterFromAngle) * val) / MaxValue;
    val += MeterFromAngle;

    var stAngle = val - ((MeterRimAngle * ThresholdMed) / MaxValue / 2);
    if (stAngle <= MeterFromAngle)
      stAngle = MeterFromAngle;

    var sweepAngle = ((MeterRimAngle * ThresholdMed) / MaxValue);
    if (stAngle + sweepAngle > MeterToAngle)
      sweepAngle = MeterToAngle - stAngle;
    ...
    context.strokeBoxedArc (x + gap, y + gap, w - gap * 2, h - gap * 2,
                            ...
                            /* counterclockwise = */ false);
    // Draw Threshold high
    context.strokeStyle = Color.threshhigh;
    context.lineWidth = Size / 50;

    var val = MaxValue - MinValue
    val = (MaxValue * (ThreshPivotHigh - MinValue)) / val;
    val = ((MeterToAngle - MeterFromAngle) * val) / MaxValue;
    val += MeterFromAngle;

    var stAngle = val - ((MeterRimAngle * ThresholdHigh) / MaxValue / 2);
    if (stAngle <= MeterFromAngle)
      stAngle = MeterFromAngle;

    var sweepAngle = ((MeterRimAngle * ThresholdHigh) / MaxValue);
    if (stAngle + sweepAngle > MeterToAngle)
      sweepAngle = MeterToAngle - stAngle;
    ...
    context.strokeBoxedArc (x + gap, y + gap, w - gap * 2, h - gap * 2,
      ...
      context.fillStyle = options.color;
      context.textAlign = 'center';
      context.font = '14pt ' + Color.meter.font;
      context.textAlignment = 'center';
      context.fillText ("cm/s", w/2+10,h-50);
